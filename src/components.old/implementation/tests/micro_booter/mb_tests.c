...
test_mem(void)
{
	...
	p = cos_page_bump_alloc(&booter_info, 1, PAGE_ORDER);
	assert(p);
	strcpy(p, chk);
	...
	s = cos_page_bump_alloc(&booter_info, 1, PAGE_ORDER);
	assert(s);
	prev = s;
	for (i = 0; i < TEST_NPAGES; i++) {
		t = cos_page_bump_alloc(&booter_info, 1, PAGE_ORDER);
	...
	t = cos_page_bump_alloc(&booter_info, TEST_NPAGES, PAGE_ORDER);
	assert(t);
	memset(t, 0, TEST_NPAGES * PAGE_SIZE);
	PRINTC("SUCCESS: Atomically allocated and zeroed %d pages.\n", TEST_NPAGES);

	t = cos_page_bump_alloc(&booter_info, 1, COS_PAGE_ORDER(1));
	assert(t);
	memset(t, 0, COS_PAGE_SIZE(1));
	PRINTC("SUCCESS: Atomically allocated and zeroed %d super pages.\n", 1);

	t = cos_page_bump_alloc(&booter_info, 2, COS_PAGE_ORDER(1));
	assert(t);
	memset(t, 0, 2 * COS_PAGE_SIZE(1));
	PRINTC("SUCCESS: Atomically allocated and zeroed %d super pages.\n", 2);
}
