...
extern const int pos2order[];
/* Page sizes */
#define COS_PAGE_ORDER(X) (pos2order[X])
#define COS_PAGE_SIZE(X) (1 << COS_PAGE_ORDER(X))
...
struct cos_meminfo {
	...
	vaddr_t    untyped_heap_frontier[COS_PGTBL_NUM_ORDER], untyped_bump_frontier[COS_PGTBL_NUM_ORDER];
};
...
struct cos_compinfo {
	...
	vaddr_t heap_frontier[COS_PGTBL_NUM_ORDER], bump_frontier[COS_PGTBL_NUM_ORDER];
...
vaddr_t cos_pgtbl_intern_alloc(struct cos_compinfo *ci, pgtblcap_t cipgtbl, vaddr_t mem_ptr, size_t num, size_t order);
/*
...
void *cos_page_bump_alloc(struct cos_compinfo *ci, size_t num, size_t order);
...
vaddr_t cos_mem_aliasn(struct cos_compinfo *dstci, struct cos_compinfo *srcci, vaddr_t src, size_t num, size_t order);
int     cos_mem_alias_at(struct cos_compinfo *dstci, vaddr_t dst, struct cos_compinfo *srcci, vaddr_t src, size_t order);
...
void   *cos_hw_map(struct cos_compinfo *ci, hwcap_t hwc, paddr_t pa, size_t num, size_t order);
