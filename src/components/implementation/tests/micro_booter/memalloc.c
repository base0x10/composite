...
void TLSF_List_Crt(volatile struct TLSF_List* Head)
{
    Head->Prev=(struct TLSF_List*)Head;
    Head->Next=(struct TLSF_List*)Head;
}

void TLSF_List_Del(volatile struct TLSF_List* Prev,volatile struct TLSF_List* Next)
{
    Next->Prev=(struct TLSF_List*)Prev;
    Prev->Next=(struct TLSF_List*)Next;
}

void TLSF_List_Ins(volatile struct TLSF_List* New,
                   volatile struct TLSF_List* Prev,
                   volatile struct TLSF_List* Next)
{
...
ret_t TLSF_Mem_Init(volatile void* Pool, ptr_t Size)
{
    cnt_t FLI_Cnt;
    ptr_t Offset;
    ptr_t Bitmap_Size;
    ...
    for(FLI_Cnt=0;FLI_Cnt<(cnt_t)(Bitmap_Size>>TLSF_ALIGN_ORDER);FLI_Cnt++)
...
void* TLSF_Malloc(volatile void* Pool, ptr_t Size)
{    
    cnt_t FLI_Level;
    cnt_t SLI_Level;
    volatile struct TLSF_Mem* Mem;
...
void TLSF_Free(volatile void* Pool, void* Mem_Ptr)
{
    volatile struct TLSF_Mem* Mem;
    volatile struct TLSF_Mem_Head* Mem_Head;
    ...
    if((ptr_t)Mem_Head!=Mem->Start)
    {
        ...
        if(Left_Head->State==TLSF_MEM_FREE)
        {
            ...
            _TLSF_Mem_Block(Left_Head,
                           (ptr_t)((ptr_t)(Mem_Head->Tail)+sizeof(struct TLSF_Mem_Tail)-(ptr_t)Left_Head));
...
void* TLSF_Realloc(volatile void* Pool, void* Mem_Ptr, ptr_t Size)
{
            ...
            if(Right_Head->State==TLSF_MEM_FREE)
            {
                ...
                if((((ptr_t)Right_Head->Tail)-((ptr_t)Mem_Ptr))>=Rounded_Size)
                {
                    ...
                    else
                    {
                        ...
                        TLSF_COVERAGE_MARKER();
                        Old_Size=((ptr_t)(Right_Head->Tail))-((ptr_t)Mem_Head)+sizeof(struct TLSF_Mem_Tail);
                        _TLSF_Mem_Block(Mem_Head, Old_Size);
                    }
